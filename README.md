...
基础，直接加入了 Green Engineering Menu、B3-OPT v52 运行链路、故障恢复、
...
docs/V52_CLOCK_HOST_FIX.md                 v52 长时退出修复说明（中英双语）
```
...
- v52 修复 clock host 的 `LD_PRELOAD` 继承：主 shell 只加载一次镜像库，子
  `/bin/sleep` 不再重复创建 worker。该问题曾在约 19 分半后使 clock host 退出；
- v52 保留已验证能启动 native server 的 `sh -c` clock host；
