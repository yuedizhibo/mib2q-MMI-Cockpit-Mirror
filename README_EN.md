...
This project is based on [jilleb/mib2-toolbox](https://github.com/jilleb/mib2-toolbox) and adds the Green Engineering Menu, the B3-OPT v52 renderer-local full-screen mirror path, fail-safe recovery, log recording, a real full-chain compatibility CHECK, persistent AutoStart, and complete uninstall/restore.
...
Normal START remains restricted to the validated train. `CHECK` does **not** decide compatibility from the model or firmware name alone; it performs a controlled real B3 run. It also refuses to write an unverified Java controller to an unknown firmware merely for experimentation.
...
CHECK is a real full-chain compatibility probe, not a train-name lookup. It reuses the same B3 v52 worker as START in a finite-duration mode and requires evidence for the complete path:

1. locked native package validation;
2. real clock-host startup;
3. a real renderer-local `1024×480` MMI frame;
4. a real EGL submission;
5. the production `ARMED` / `receiver_ready` handoff;
6. Java controller state `ACTIVE`;
7. explicit `nativeContext=76 source=58 fps=30` confirmation;
8. a short stable ACTIVE window;
9. marker removal, Java return to `IDLE`, and stock-route recovery.

Only a complete pass prints:
...
Any missing critical stage prints:
...
The failure stage is reported explicitly, for example `clock-host`, `mmi-capture-1024x480`, `renderer-egl`, `java-displaymanager-context76`, or `safe-restore`. Detailed results are saved to:
...
> [!IMPORTANT]
> CHECK briefly runs the real B3 route, so the cockpit may temporarily show the mirrored MMI during the test. Run it only while parked.

On the validated P1404 train, if `Cockpit_Mirror.jar` is not installed yet, the first CHECK safely prepares the same verified controller used by START and returns `REBOOT REQUIRED`. After one complete MMI reboot, run CHECK again for the final CAN USE / CANNOT USE result. On unknown trains CHECK will not install a Java JAR just to experiment.
...
- removes the mirror-specific Green Menu and CHECK/B3/AutoStart scripts;
...
4. You may run CHECK first. On P1404, if the controller is not yet installed, the first CHECK prepares `Cockpit_Mirror.jar` and requests a complete reboot; the second CHECK performs the real full-chain probe.
5. The first START or AutoStart ON performs the same controller preparation when required.
6. After reboot, select START for manual mirroring or let AutoStart run automatically if enabled.
7. The current mirror runtime, CHECK, and AutoStart require the Toolbox SD card to remain inserted.
...
- CHECK uses the same production worker in finite-duration mode and automatically restores after the probe;
...
Persistent logs are bounded. Each START/CHECK truncates the current B3 active logs, and the watchdog rotates high-volume logs when they exceed 2 MiB, retaining one `.previous` copy.

## Repository layout

```text
Toolbox/GEM/mqb-mmiCockpitMirror.esd                 Green Menu screen
Toolbox/scripts/check_cockpit_mirror_compatibility.sh full-chain CHECK
Toolbox/scripts/start_direct_upload_test.sh           B3 START
Toolbox/scripts/stop_direct_upload_test.sh            B3 STOP
Toolbox/scripts/direct_upload_test_worker.sh           shared persistent/finite B3 worker
...
- the 2026-09-05 CHECK update adds a real B3 full-chain probe that only passes when capture, EGL, Java context76/source58 activation, and safe recovery all succeed;
...
The primary additions in this repository include renderer-local full-MMI texture capture, displayable-58 output, persistent 30-FPS control, VIEW recovery, PID watchdog behavior, in-car logging/rollback, the real full-chain CHECK, AutoStart, and complete mirror uninstall management.
